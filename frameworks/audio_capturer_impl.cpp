...
uint64_t AudioCapturer::AudioCapturerImpl::GetFrameCount()
{
    ...
    if (audioSource_ == nullptr) {
        MEDIA_ERR_LOG("audioSource_ null");
        return 0;
    }
...
static void SetSourceConfigInfo(const AudioCapturerInfo info, uint32_t deviceId, AudioSourceConfig &sourceConfig)
{
...
static void SetEncoderConfigInfo(const AudioCapturerInfo info, AudioEncodeConfig &encodeConfig)
{
...
int32_t AudioCapturer::AudioCapturerImpl::SetCapturerInfo(const AudioCapturerInfo info)
{
    ...
    SetSourceConfigInfo(info, devices[0].deviceId, sourceConfig);
    ...
    if (info.audioFormat != PCM && info.audioFormat != AUDIO_DEFAULT) {
        ...
        SetEncoderConfigInfo(info, encodeConfig);
...
int32_t AudioCapturer::AudioCapturerImpl::Read(uint8_t *buffer, size_t userSize, bool isBlockingRead)
{
    ...
    if (status_ != RECORDING) {
        MEDIA_ERR_LOG("not RECORDING status_:%u", status_);
        return ERR_INVALID_READ;
    }
...
bool AudioCapturer::AudioCapturerImpl::Release()
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (status_ == RELEASED) {
        MEDIA_ERR_LOG("check state:%u error", status_);
        return false;
    }
    if (status_ == INITIALIZED) {
        status_ = RELEASED;
